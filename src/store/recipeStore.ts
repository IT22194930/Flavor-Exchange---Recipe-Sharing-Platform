...
export const useRecipeStore = create<RecipeState>()(persist((set, get) => ({
  ...
  fetchUserRecipes: async (userId: string) => {
    ...
    try {
      const userRecipes = mockRecipes.filter(recipe => recipe.author.id === userId);
      set({
        userRecipes,
        isLoading: false
      });
  ...
  updateRecipe: async (id, recipeData) => {
    ...
    try {
      const updatedRecipes = get().recipes.map(recipe => recipe.id === id ? {
        ...recipe,
        ...recipeData
      } : recipe);
      set({
  ...
  toggleFavorite: (userId: string, recipeId: string) => {
    set(state => {
      ...
      const updatedUserFavorites = isFavorite ? userFavorites.filter(id => id !== recipeId) : [...userFavorites, recipeId];
      return {
        favorites: {
          ...state.favorites,
  ...
  searchRecipes: query => {
    ...
    return get().recipes.filter(recipe => recipe.title.toLowerCase().includes(lowerQuery) || recipe.ingredients.some(ingredient => ingredient.toLowerCase().includes(lowerQuery)));
  },
